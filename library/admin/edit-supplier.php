...
$add=$_POST['address'];
$tpno=$_POST['tpno'];
$sql="update  tblsuppliers set supname=:supname where supid=:supid";
...
<?php 
$supid=intval($_GET['supid']);
$sql = "SELECT * from  tblsuppliers where supid=?";
$query = $dbh -> prepare($sql);
$query->bindParam(':supid',$supid,PDO::PARAM_STR);
$query->execute();
...
{               ?>   
    <div class="form-group">
        <input class="form-control" type="text" name="supname" value="<?php echo htmlentities($results->supname);?>" required />
    </div>
